...
This site has been through a few iterations. What started as a vague idea of "I want a personal website" turned into a full rebuild, and honestly, most of it was written with AI help. The result is an Astro-powered site living on GitHub Pages, with just enough custom plumbing to make it feel like mine.
...
Then `npm run publish:post` or `npm run publish:photo` to finalize. It beats manually creating markdown files with the right frontmatter every time.

Photos get copied into `public/images/photos/`, metadata lives in `src/content/photos/`, and everything is file-based. Posts work the same way: content is just markdown in a folder.
...
Click any photo and you get a lightbox-style viewer with zoom controls, panning, and a full-size link. It works on desktop; on mobile it gracefully falls back to the normal photo page. The whole thing is built into the site with no external libraries for the viewer.
